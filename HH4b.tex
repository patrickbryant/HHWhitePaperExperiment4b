...
Events a required to contain four \btagged jets with $p_T>40\,(30)\,$GeV in the most recent ATLAS \cite{Aaboud:2018knk} (CMS \cite{Sirunyan:2018tki}) results.
The \btagging efficiency is 70\%,(65\%) with a light jet mistag efficiency of 0.3\%\,(1\%) for ATLAS \cite{ATL-PHYS-PUB-2016-012} (CMS \cite{CMS-DP-2016-042}).
The dominant background processes after this preselection are multi-\bjet production (90\%) and top quark pair production (10\%).
...
\begin{figure}
  \begin{center}
    ...
    \subfloat[2018 Analysis \cite{Aaboud:2018knk}
...
The background modeling approach taken by the ATLAS collaboration since the beginning has been to model high \bjet multiplicity events with low \bjet multiplicity events.
...
%% CMS: Functional fit for resonance searches \cite{Sirunyan:2018zkk} (this paper also quantifies impact of b-jet energy regression) and hemisphere mixing for non resonant HH search \cite{Sirunyan:2018tki}.
...
For their non-resonant search CMS developed a novel 'hemisphere mixing' technique whereby fake events are generated by mixing and matching dijet systems from separate events \cite{Sirunyan:2018tki}.
...
The recent ATLAS result \cite{Aaboud:2018knk} attempted to address this by deriving the background model twice
...
With the 27.5\,\ifb modeled in \cite{Aaboud:2018knk} one could argue hints of such effects are becoming visible and a new strategy will almost certainly be required for analysis of the full Run-2 data set. 
...
All final states with \bjets stand to gain from dedicated \bjet energy regressions and calibrations.
In the \bbbb case improvements in the \bjet energy scale reduce the mass resolution for both Higgs bosons allowing for tighter signal region definitions with the same signal efficiency.
CMS has demonstrated this in their most recent resonant search \cite{Sirunyan:2018zkk} where the Higgs mass resolution for different resonance mass hypotheses improved by 6-12\%.
The tighter optimal signal region definitions then improved the search sensitivity by 5-20\%.
...
ATLAS has already released a search for Higgsino pair production \cite{PhysRevD.98.092002} using a BDT based reweighting scheme using the same event selection as \cite{Aaboud:2018knk}.
